...
router.post('/', authUtil, async (req, res) => {
    ...
    try {
        ...
        const query = `SELECT s.* FROM song s
        INNER JOIN mypage m ON s.Index = m.Index
        WHERE s.\`Index\` in (SELECT \`Index\` FROM mypage WHERE nickname = ? ORDER BY song_date DESC)
        ORDER BY m.song_date DESC`;
